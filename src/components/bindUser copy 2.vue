  ...
  <div id="bg">
    ...
    <div class="login" style="overflow: hidden;">
      <!-- <wx-open-launch-weapp
                    id="launch-btn"
                    username="gh_74f53a4a556c"
                    path="/pages/packageA/pages/ChatFree/main.html"
                    >
                        <script type="text/wxtag-template">
                        <style>.btn { padding: 12px }</style>
                        <button class="btn">与医生聊天</button>
                    </script>
      </wx-open-launch-weapp>-->
    ...
    <el-dialog title="提示" :visible.sync="dialogVisible" :close-on-click-modal="false" width="77%">
  ...
  methods: {
    showExitModal() {
      ...
      let intervalTime = setInterval(function() {
        ...
        if (that.time > 1) {
          ...
          //下面weixin退出去方法
          this.params_key = this.getUrlParam("key");
          this.params_diagnoseId = this.getUrlParam("diagnoseId");
          this.params_doctorId = this.getUrlParam("doctorId");
          wx.miniProgram.navigateTo({
            url:
              "/pages/packageA/pages/ChatFree/main?key=" +
              that.params_key +
              "&diagnoseId=" +
              that.params_diagnoseId +
              "&doctorId=" +
              that.params_doctorId
          });
          // wx.closeWindow();
    ...
    async sginWexinsdk() {
      ...
      const response = await this.$apollo.mutate({
        mutation: gql`
          mutation($noncestr: String!, $timestamp: String!, $url: String!) {
            wxJsapiSign(noncestr: $noncestr, timestamp: $timestamp, url: $url)
          }
        `,
        variables: {
          noncestr: noncestr,
